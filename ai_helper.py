...
"""
    ...
    def transcribe_audio_to_text(self, audio_file_path: str, context_info: Optional[str] = None) -> Tuple[bool, str]:
        ...
        try:
            ...
            # プロンプトを設定
            prompt = """この音声は放課後等デイサービスの朝礼会議です。音声の内容を正確にテキストに変換してください。

変換のポイント：
- 話し手の言葉をそのまま記録し、言いよどみや繰り返しも含めて正確に書き起こしてください
- スタッフの名前、利用者の名前、施設名、地名などの固有名詞を正確に記録してください
- 朝礼特有の用語（送迎、バイタル、集団活動など）を正確に書き起こしてください
- 不要な編集は行わず、話された内容を忠実に記録してください
- 時系列で記録し、話者の切り替わりを明確に区別してください"""
...
これらの情報を参考にしながら、音声の内容を正確にテキストに変換してください。"""
    ...
    def categorize_meeting_content(self, text: str) -> Dict[str, str]:
        """
        朝礼の内容をキーワードベースで分類する補助関数
        ...
        patterns = {
            "議題・内容": [
                "業務内容", "利用者", "児童", "送迎", "活動", "体調", "様子",
                "報告", "予定", "今日", "昨日", "今週", "来週", "集団", "個別",
                "バイタル", "食事", "水分", "排泄", "学習", "遊び", "特記事項"
            ],
            "決定事項": [
                "決定", "方針", "対応", "対策", "実施", "変更", "採用",
                "承認", "同意", "了承", "結論", "取り決め"
            ],
            "共有事項": [
                "共有", "注意", "確認", "連絡", "お知らせ", "情報", "周知",
                "リマインド", "依頼", "お願い", "伝達", "アナウンス"
            ]
        }

        result = {"議題・内容": "", "決定事項": "", "共有事項": "", "その他メモ": ""}
    ...
    def generate_meeting_minutes_from_text(self, text: str) -> Tuple[bool, Dict[str, str]]:
        ...
        try:
            ...
            prompt = f"""この音声は、放課後等デイサービスの朝礼会議の音声です。この音声をもとに朝礼議事録を作成してください。
音声に含まれていない情報をでっち上げたり、冗長になったりしてはいけません。

朝礼では、スタッフ間で以下の内容が共有・議論されます：
- 前日の業務内容と利用者の状況
- 今日の業務予定と役割分担
- 利用者の体調変化や行動パターン
- 施設運営上の課題と解決策
...
{{
    "議題・内容": "朝礼で議論された具体的な内容を時系列または論理的順序でまとめ、MECE原則に基づいて記載してください。業務報告、利用者状況、予定確認、問題提起などを含みます。",
    "決定事項": "朝礼で決定・合意された具体的な事項を箇条書きで記述してください。対応方針、役割分担、対策方法などが該当します。なければ空文字列にしてください。",
    "共有事項": "スタッフ間で共有すべき情報や連絡事項を箇条書きで記述してください。注意喚起、連絡事項、参考情報などが該当します。なければ空文字列にしてください。",
    "その他メモ": "議事録の補足情報や詳細内容、フォローアップ事項を記述してください。議事の流れや重要なポイントの詳細、検討中の事項などが該当します。なければ空文字列にしてください。"
}}

注意事項:
- 放課後等デイサービスの業務文脈を考慮してください
- 音声の内容を正確に反映し、重要な情報を見落とさないようにしてください
- 各項目は明確に分類し、読みやすく整理してください
- 内容はMECE原則に基づき、重複なく分類してください
- 音声に含まれていない情報をでっち上げてはいけません
...
"""
    ...
    def generate_title_from_text(self, text: str) -> tuple:
        ...
        prompt = f"""このテキストは放課後等デイサービスの朝礼議事録の内容です。この内容から適切なタイトルを生成してください。

タイトル生成のポイント：
- 朝礼で議論された主要な話題を反映したタイトルにすること
- 20文字以内で簡潔にまとめること
- 必ず「の件」という形式で終わること
- 利用者関連、業務関連、施設関連などのキーワードを適切に抽出すること

入力テキスト：
{text}

出力はタイトルのみ（「○○の件」形式）で、説明文は一切不要です。"""
